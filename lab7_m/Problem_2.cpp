...
void lb_litmus_test_tso(int iteration, memory_order mem_order)
{
    x.store(0);
    y.store(0);

    thread t1([&]
              { r1.store(x.load(mem_order), mem_order); y.store(1, mem_order); });
    thread t2([&]
              { r2.store(y.load(mem_order), mem_order); x.store(1, mem_order); });
...
void sb_litmus_test_tso(int iteration, memory_order mem_order)
{
    x.store(0);
    y.store(0);

    thread t1([&]
              { x.store(1, mem_order); r1.store(y.load(mem_order), mem_order); });
    thread t2([&]
              { y.store(1, mem_order); r2.store(x.load(mem_order), mem_order); });
...
void mp_litmus_test_tso(int iteration, memory_order mem_order)
{
    x.store(0);
    y.store(0);

    thread t1([&]
              { x.store(1, mem_order); y.store(1, mem_order); });
    thread t2([&]
              { r1.store(y.load(mem_order), mem_order); r2.store(x.load(mem_order), mem_order); });
...
void iriw_litmus_test_tso(int iteration, memory_order mem_order)
{
    x.store(0);
    y.store(0);
    ...
    thread t2([&]
              { y.store(1, mem_order); });
    thread t3([&]
              { r1.store(x.load(mem_order), mem_order); r2.store(y.load(mem_order), mem_order); });
    thread t4([&]
              { r3.store(y.load(mem_order), mem_order); r4.store(x.load(mem_order), mem_order); });
...
int main()
{
    ...
    {
        lb_litmus_test_tso(i, mem_order);
    ...
    ofstream csv_file("Problem_2_Output.csv");
    ...
    for (int i = 0; i < 10000; ++i)
    {
        sb_litmus_test_tso(i, mem_order);
    ...
    for (int i = 0; i < 10000; ++i)
    {
        mp_litmus_test_tso(i, mem_order);
    ...
    for (int i = 0; i < 10000; ++i)
    {
        iriw_litmus_test_tso(i, mem_order);
        r[r1][r2][r3][r4]++;
    }
